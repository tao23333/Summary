...
- 创建虚拟环境
  ...
  - 补充：
    ...
    > - 进入环境：`workon ai`
    > - 查看环境：`workon`
    >   - 如果在VNC终端中显示workon：未找到命令，则再执行`source ~/.profile`一次
...
>
> 可以发现上面有3个可执行文件，选择任意一个都可以。python版本都和创建虚拟环境时指定的版本一致。同时，在同一个虚拟环境中，使用pip或者pip3效果都是一样的
...
- > 也可以先进入虚拟环境，然后使用命令：`python Desktop/test.py`，与上述命令等价
